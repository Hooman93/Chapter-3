...
# -------------------------
import os                                   # Filesystem utilities: join paths, create folders
import re                                   # Regular expressions: parse labels out of paths
from pathlib import Path                    # Safe/clear path handling (optional, kept for parity)
import numpy as np                          # Numerical workhorse
import pandas as pd                         # Tabular I/O (Excel/CSV)
import matplotlib.pyplot as plt             # Plotting
import matplotlib.colors as mcolors         # Color utilities (to darken marker edges)
import lvpyio as lv                         # PTV track reader for DaVis .set

# -------------------------
# User inputs (edit below)
# -------------------------

# List of input folders, each pointing to one PTV dataset (.set directory)
data_paths = [
...
# Where to save all figures and tables
output_root = r"C:\python files\Hydrogel_Three-phase\Figures\New results\Velocity distribution anlayis_varying solid VOF\0.5"
os.makedirs(output_root, exist_ok=True)     # Create output folder tree if needed (no error if exists)
...
def extract_svf_from_label(label):
    """
    Extract only the solid VOF percentage from a full case label, e.g.
    '30% | 0.05 L/min' -> '30%'.
...
def plot_vmag_boxplots(vmag_by, title, out_stub):
    ...
    - One box per case label (effect of solids at given gas flow rate).
    - X-axis shows only solid VOF (0, 10, 30, 50%).
    ...
    # Extract only the solid VOF part (e.g. '0%', '10%', '30%', '50%') for x-axis
    display_labels = [extract_svf_from_label(lbl) for lbl in case_labels]
    ...
    ax1.set_xlabel("Solid VOF (%)")                       # X-axis = solid volume fraction
...
def process_all_cases(data_paths):
    ...
    for case_path in data_paths:
        ...
        # Keep positions (MSD removed, but we retain variable for minimal change)
...
plot_vmag_boxplots(
    vmag_by,
    "|V| distribution across cases (boxplots)",
    "Boxplot_Vmag_across_cases"
)
